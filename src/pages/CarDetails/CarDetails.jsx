...
import { LuMapPin, LuCircleCheck, LuCalendar, LuCar, LuFuel, LuGauge, LuInfo } from "react-icons/lu";
...
export default function CarDetails() {
  ...
  if (loading && !car) return <Loader type="page" label="Loading car…" />;
  if (!loading && !car) return <p>Car not found.</p>;
            ...
            <div className={css.infoCar}>
              <div className={css.oneInfo}>
                ...
                <div className={css.id}>
                  <span>Id:</span> {firstFourId(car.id)}
                </div>
              </div>
              ...
              <div className={css.oneInfo}>
                {(city || country) && (
                  <div className={css.address}>
                    <LuMapPin className={css.icon} />
                    <span>{[city, country].filter(Boolean).join(" | ")}</span>
                  </div>
                )}
                {car.mileage != null && (
                  <div className={css.address}>
                    <span>Mileage:</span> {formatMileage(car.mileage)}
                  </div>
                )}
              </div>
              ...
              {car.description && <p className={css.text}>{car.description}</p>}
            </div>
                  ...
                  <ul className={css.detailsList}>
                    {conditions.map((c, i) => (
                      <li key={i} className={css.detailsItem}>
                        <LuCircleCheck className={css.icon} />
                        <span>{c}</span>
                      </li>
                    ))}
                  </ul>
                ...
                <div>
                  ...
                  <ul className={css.detailsList}>
                    {specs.map((s, i) => {
                      ...
                      return (
                        <li key={i} className={css.detailsItem}>
                          <Icon className={css.specIcon} aria-hidden="true" />
